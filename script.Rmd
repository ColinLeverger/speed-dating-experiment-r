...
The dataset is provided with its key, which is a Word document you will need to quickly go through to understand properly my work.
...
## Inputations with missing values
...
The question is: is there a pattern? Can we impute, can we clean this mess?
...
It appears that nothing very interesting can be deducted from this. Indeed, most of the missing values are preferences of the people considered. Impossible to inpute that!
...
There is 10 missing `pid` (partner’s iid number) in the dataset. Since there is no `iid` missing, we could probably inpute quite easily. 
...
We can not use this column to analyse the data. Indeed, all the values in this column were directly given by the users, and the latter may write anything, including nonsence values such as "I dont know".
...
# Find date occurence for men/women
...
# Find go out occurence for men/women
...
In this dataset, there is connections between people. It fits well for a graph analysis, and we will thus use Gephy to do so.
